...
void cleanup(struct ring_buffer *rb, struct sched_all_bpf *skel){
    if (rb) ring_buffer__free(rb);
    if (skel) sched_all_bpf__destroy(skel);
}
...
int main(int argc, char **argv){
    ...
    while(!exiting){
        ...
        if(now - last_print_time >= 1){ 
            ...
            if(count > 0){
                printf("Collected %ld SCHEDULER events in last 1s\n", count);
            }
            last_print_time = now;
        }
    }
